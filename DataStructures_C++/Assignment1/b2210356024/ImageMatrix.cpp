...
ImageMatrix ImageMatrix::operator+(const ImageMatrix &other) const {
    ...
    // Assume two matrices has same height and width
...
ImageMatrix ImageMatrix::operator-(const ImageMatrix &other) const {
    ...
    // Assume two matrices has same height and width
    ImageMatrix result(height, width);
    if(other.get_height() == height && other.get_width() == width) {
            for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            result.data[i][j] = data[i][j] - other.data[i][j];
            }
...
void ImageMatrix::set_data(int i, int j, double value) {
        // Assuming 'data' is a 2D array storing pixel values
        // Check if the indices are within the image bounds
        if (i >= 0 && i < height && j >= 0 && j < width) {
            // Set the pixel value at position (i, j) to the specified 'value'
            data[i][j] = value;
        }
    }

// Getter func to access height
int ImageMatrix::get_height() const {
    return height;
}

// Getter func to access width
int ImageMatrix::get_width() const {
    return width;
}

// Add this member function to your ImageMatrix class
void ImageMatrix::write_to_file(const std::string &filename) {
    std::ofstream file(filename); // Create a file stream for writing
